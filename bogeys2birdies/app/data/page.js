...
import { Eyebrow, PageHero, Stat } from '../../components/UI';
...
export default async function Data() {
  ...
  return <Page siteSettings={site}><main className="inner-page"><PageHero page={page}/><section className="section-shell data-page"><div className="stats-grid">{page.stats?.map((stat) => <Stat key={stat._key || stat.label} {...stat} />)}</div><div className="chart-card"><div className="chart-heading"><div><Eyebrow>{page.chartEyebrow}</Eyebrow><h2>{page.chartHeading}</h2></div><strong>{Number.isFinite(Number(currentHandicap)) ? Number(currentHandicap).toFixed(1) : currentHandicap}</strong></div><HandicapTrend milestones={page.handicapMilestones} currentHandicap={currentHandicap} /></div>{season && <div className="season-dashboard"><section><div className="section-heading"><Eyebrow>MONTH BY MONTH</Eyebrow><h2>How the season is moving.</h2></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Month</th><th>Rounds</th><th>Avg.</th><th>GIR</th><th>Fairways</th><th>Putts</th><th>Scrambling</th><th>Penalties</th></tr></thead><tbody>{season.monthlyCheckpoints?.map((r)=><tr key={r._key}><td data-label="Month">{r.month}</td><td data-label="Rounds">{r.roundsPlayed}</td><td data-label="Average">{r.averageGross?.toFixed(1)}</td><td data-label="GIR">{r.girPercent}%</td><td data-label="Fairways">{r.fairwaysHitPercent}%</td><td data-label="Putts">{r.averagePutts?.toFixed(1)}</td><td data-label="Scrambling">{r.scramblingPercent}%</td><td data-label="Penalties">{r.penaltiesPerRound?.toFixed(1)}</td></tr>)}</tbody></table></div></section><section><div className="section-heading"><Eyebrow>SCORING PROFILE</Eyebrow><h2>Where the shots go.</h2></div><div className="season-card-grid">{season.parScoring?.map((r)=><article className="season-card" key={r._key}><span>PAR {r.par}</span><strong>{r.averageToPar>=0?'+':''}{r.averageToPar?.toFixed(1)}</strong><p>Average to par across {r.holesPlayed} holes</p></article>)}</div></section><section><div className="section-heading"><Eyebrow>RECENT ROUNDS</Eyebrow><h2>The latest scorecards.</h2></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Date</th><th>Course</th><th>Gross</th><th>To par</th><th>Stableford</th></tr></thead><tbody>{season.recentRounds?.map((r)=><tr key={r._key}><td data-label="Date">{new Intl.DateTimeFormat('en-GB',{day:'numeric',month:'short',timeZone:'UTC'}).format(new Date(r.date))}</td><td data-label="Course">{r.courseName}</td><td data-label="Gross">{r.gross}</td><td data-label="To par">{r.toPar>=0?'+':''}{r.toPar}</td><td data-label="Stableford">{r.stablefordPoints}</td></tr>)}</tbody></table></div></section><p className="season-source">Based on {season.roundCount} rounds · {season.homeClub} · Updated {new Intl.DateTimeFormat('en-GB',{day:'numeric',month:'long',year:'numeric',timeZone:'UTC'}).format(new Date(season.exportedAt))}</p></div>}</section></main></Page>;
}
