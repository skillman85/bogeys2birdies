import { Page } from '../../components/SiteChrome'; import { Eyebrow, ExperimentCard } from '../../components/UI';
...
export default async function Experiments(){const [items,site,page]=await Promise.all([getExperiments(defaultExperiments),getSiteSettings(defaultSiteSettings),getPageSettings('experiments',defaultPageSettings.experiments)]);return <Page siteSettings={site}><main className="inner-page"><section className="page-hero"><Eyebrow>{page.eyebrow}</Eyebrow><h1>{page.title}</h1><p>{page.description}</p></section><section className="section-shell"><div className="story-grid light-cards">{items.map((item)=><ExperimentCard key={item._id||item.slug} {...item} image={imageUrl(item.coverImage)}/>)}</div></section></main></Page>}
