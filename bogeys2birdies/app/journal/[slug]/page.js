...
import { getContentDetail, getRelatedArticles, getSiteSettings } from '../../../sanity/lib/content';
...
export default async function Article({ params }) { const { slug } = await params; const [item, site] = await Promise.all([getContentDetail('article', slug), getSiteSettings(defaultSiteSettings)]); if (!item) notFound(); const relatedItems = await getRelatedArticles(item.slug, item.categorySlug); return <ContentDetail item={item} site={site || defaultSiteSettings} eyebrow={item.category || 'Journal'} eyebrowSlug={item.categorySlug} meta={formatArticleMeta(item)} relatedItems={relatedItems} />; }
